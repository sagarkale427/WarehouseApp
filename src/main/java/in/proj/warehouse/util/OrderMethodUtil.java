...
public class OrderMethodUtil {
	...
	public void generatePieChart(String path,List<Object[]> data) {
		...
		//3.save as image
		try {
		ChartUtils.saveChartAsJPEG(new File(path+"/omA.jpg"),  //file location +name  
	...
	public void generateBarChart(String path,List<Object[]> data) {
		...
		//2.create JFree Chart object
		//Input=>title, dataset
		JFreeChart chart=ChartFactory.createBarChart("SHiPMENT TYPE MODE","MODE","COUNT", dataSet);
		
		//3.save as image
		try {
		ChartUtils.saveChartAsJPEG(new File(path+"/omB.jpg"),  //file location +name  
				chart,    //JFreeChart object
				400,      //width
				350);     //height
		}catch(IOException e) {
